locals {
  imageId = var.useTenantImage ? data.openstack_images_image_v2.terraform_tenant_image.id : openstack_images_image_v2.terraform_upstream_image.id
}
...
resource "openstack_compute_instance_v2" "terraform_deploy_instance" {
  ...
  depends_on = [
    openstack_networking_router_v2.terraform_deploy_router,
    openstack_networking_subnet_v2.terraform_deploy_network_sub1,
  ]

  security_groups = [
    openstack_networking_secgroup_v2.terraform_remote_secgroup.name,
